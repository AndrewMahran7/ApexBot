#!/usr/bin/env python3
"""
Apex Ã¢â‚¬â€ Main System Runner
===========================

Wires the full pipeline:
    data Ã¢â€ â€™ StrategyEngine Ã¢â€ â€™ RiskManager Ã¢â€ â€™ Execution (Paper | Tradovate)

Modes:
    replay  Ã¢â‚¬â€ Feed historical CSV bars through the pipeline.
    paper   Ã¢â‚¬â€ Same as replay but with PaperEngine tracking execution.
    live    Ã¢â‚¬â€ Connect to Tradovate and feed bars from a live source.
...
from execution.tradovate_client import TradovateClient, TradovateConfig
...
def setup_logging(log_dir: str = "logs", level: str = "INFO") -> None:
    ...
    # Signals Ã¢â‚¬â€ all strategy signals
    ...
    # Errors Ã¢â‚¬â€ WARNING and above
...
def run_bar_loop(
    ...
    for i, (ts, row) in enumerate(bars.iterrows()):
        ...
        # Order matters: prop Ã¢â€ â€™ risk Ã¢â€ â€™ paper Ã¢â€ â€™ engine
...
def run_live(pipeline: dict) -> None:
    ...
    try:
        ...
        logger.info("Live mode active Ã¢â‚¬â€ waiting for bars (Ctrl+C to stop)")
...
def print_summary(pipeline: dict, mode: str) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"  Apex Run Summary Ã¢â‚¬â€ mode={mode}")
    print("=" * 60)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Apex Ã¢â‚¬â€ MES Futures Trading System Runner",
    )
    ...
    # Pre-flight validation (live mode)
    p.add_argument("--skip-preflight", action="store_true",
                   help="Skip pre-flight validation (NOT recommended for live)")
    p.add_argument("--expected-symbol", default="",
                   help="Expected active symbol in Tradovate (e.g. MNQ, MES)")
    p.add_argument("--expected-account", default="",
                   help="Expected account label substring (e.g. APEX-12345)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("Apex starting Ã¢â‚¬â€ mode=%s", args.mode)
    ...
    logger.info(
        "Pipeline wired: StrategyEngine Ã¢â€ â€™ RiskManager Ã¢â€ â€™ %s",
    ...
    if args.mode in ("replay", "paper"):
        ...
        # --- Pre-flight validation (live mode only) ---
        if not getattr(args, 'skip_preflight', False):
            from execution.preflight import run_preflight
            preflight = run_preflight(
                expected_symbol=getattr(args, 'expected_symbol', ''),
                expected_account=getattr(args, 'expected_account', ''),
            )
            preflight.print_report()
            if not preflight.passed:
                logger.error("Pre-flight validation FAILED — aborting live startup")
                return 1
        else:
            logger.warning("Pre-flight validation SKIPPED (--skip-preflight)")

        run_live(pipeline)

    logger.info("Apex finished Ã¢â‚¬â€ mode=%s", args.mode)
    return 0
