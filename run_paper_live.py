...
full pipeline: StrategyEngine → RiskManager → PaperEngine.

Outputs:
    logs/paper_trading.log   — formatted real-time trade/signal log
    results/paper_trades.csv — all executed paper trades
...
def setup_paper_logging(log_dir: str = "logs", level: str = "INFO") -> None:
    ...
    # Console — real-time paper trading output
    ...
    # Paper trading log — the key output file
...
def build_live_pipeline(
    ...
    # Execution callback — logs formatted trade output
    ...
    # Signal callback chain — risk → execution
...
def run_paper_live(
    ...
    def _handle_sigint(signum, frame):
        nonlocal shutdown
        paper_logger.info("Shutdown signal received — stopping after current bar")
        shutdown = True

    signal.signal(signal.SIGINT, _handle_sigint)

    paper_logger.info("=" * 60)
    paper_logger.info("  PAPER TRADING — LIVE SIMULATION")
    ...
    for i, (ts, row) in enumerate(bars.iterrows()):
        if shutdown:
            paper_logger.info("Shutdown — stopping at bar %d / %d", i, total)
            break
...
def print_final_report(summary: dict) -> None:
    ...
    paper_logger.info("")
    paper_logger.info("=" * 60)
    paper_logger.info("  PAPER TRADING — FINAL REPORT")
    paper_logger.info("=" * 60)
    ...
    if summary["total_trades"] > 0 and not summary["anomalies"]:
        ...
        paper_logger.info("  STATUS: ANOMALIES DETECTED — Review required")
    else:
...
class SymbolPipeline:
    """All components for a single symbol — fully independent state."""
    symbol: str
    instrument: InstrumentConfig
...
def build_multi_pipeline(
    ...
    AnalyticsEngine — no shared indicator or EMA state.
    """
    strategy_cfg = HybridEMAMLConfig(
        multi_candidate=len(args.ema_periods) > 1 or len(args.entry_types) > 1,
        ema_periods=tuple(args.ema_periods),
    ...
    for sym in symbols:
        ...
        if instrument is None:
            paper_logger.error("Unknown symbol: %s — not in INSTRUMENT_REGISTRY", sym)
            sys.exit(1)
        ...
        # Build engine — multi-strategy or EMA-only, per symbol
        intraday_cfg = None
        enable_hybrid = True
        if args.multi_strategy:
            intraday_cfg = IntradayConfig(
...
def run_paper_multi_symbol(
    ...
    paper_logger.info("=" * 60)
    paper_logger.info("  MULTI-SYMBOL PAPER TRADING — LIVE SIMULATION")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Apex — Live Paper Trading Simulation",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    paper_logger.info("Apex Paper Trading — starting")
    ...
    if args.test_telegram:
        ...
        if ok:
            ...
            paper_logger.error("Telegram test message FAILED — check token/chat_id")
            return 1
...
def _main_multi_symbol(args: argparse.Namespace) -> int:
    ...
    if args.tradovate_sim or args.tradovate_dry_run:
        ...
        if args.tradovate_dry_run:
            ...
            if results.get("_all_ok"):
                paper_logger.info("DRY RUN PASSED — all symbols validated")
            else:
                paper_logger.error("DRY RUN FAILED — fix issues above")
    ...
    # Telegram alerter (optional — degrades gracefully)
    from strategy.telegram_alerts import create_alerter
...
def _main_single_symbol(args: argparse.Namespace) -> int:
    """Original single-symbol path — unchanged behavior."""
    # Load data
    ...
    if len(bars) == 0:
        paper_logger.error("No bars loaded — check data file and date filters")
        return 1
