...
**ML filter:** A gradient-boosted classifier (trained on 35 engineered features) scores each candidate trade. ML does **not** generate signals — it only filters them. Trades below the ML threshold are rejected.

**Trade management:**
- Stop loss: opening-range low/high (direction-dependent)
- Take profit: reward-risk multiple (default 1.5×)
- End-of-day exit: all positions closed by 15:55 ET
...
Historical data and backtests reference NQ (full Nasdaq-100 futures). The execution layer maps to MNQ (Micro Nasdaq-100) for realistic micro-futures deployment. Strategy logic and ML models are unchanged — only the contract specification at the execution layer differs.

---

## Backtesting Results

**Dataset:** MES 5-minute bars, Jan 2021 – Dec 2024 (281,000 bars, ~4 years).
...
| Position Scale | Pass Rate | Fail Rate | Avg Trades to Pass | Avg Max DD |
|---|---|---|---|---|
| 0.50× | 11.5% | 1.2% | 159 | $436 |
| 0.75× | 39.6% | 9.5% | 137 | $602 |
| **1.00×** | **55.8%** | **23.6%** | **115** | **$700** |
| 1.25× | 59.9% | 34.8% | 93 | $754 |
| 1.50× | 57.0% | 42.1% | 73 | $787 |
...
| p95 max drawdown | $115 |
| p99 max drawdown | $143 |
| Source win rate | 55.1% |

> **Interpretation:** Filtering to EMA breakout strategies eliminates all simulated failures at the cost of requiring more trades to reach the profit target. The drawdown profile is dramatically better.

---

## Live Validation (Paper Trading)

**Status:** Paper trading with real-time Telegram alerts. No real capital deployed.

### 30-Day Multi-Symbol Paper Run (MES + MNQ)
...
| Avg PnL/trade | $48.53 |
| Total PnL | $921.99 |
| Portfolio equity | $10,922 (from $10,000) |
| Peak equity | $11,606 |
| Max drawdown | −$684 |
...
|---|---|---|---|---|
| MES | 7 | 57.1% | $160 | $22.87 |
| MNQ | 12 | 58.3% | $762 | $63.49 |

### Contract Sizing in Practice

| Symbol | Contracts Range | Typical | Notes |
|---|---|---|---|
| MES | 1–4 | 1 | Higher tick value → fewer contracts per risk unit |
| MNQ | 1 | 1 | Lower tick value → 1 contract at current equity |

**Important:** These are paper trading results. They simulate realistic execution (tick-based PnL, integer contracts, slippage, commission) but have not been validated with real capital. Do not treat this as proven live profitability.
...
1. Paper engine runs against live-delayed data
...
│   ├── intraday_strategies.py   # Additional intraday strategies
│   └── multi_strategy_engine.py # Multi-strategy coordinator
├── backtest/
│   ├── engine.py                # Bar-by-bar backtesting engine
│   ├── metrics.py               # Trade metrics and CSV export
...
│   ├── splits.py                # Time-based train/test splits
...
├── tests/                       # 528 tests
│   ├── test_paper_engine.py     # 52 tests (incl. risk-based sizing)
│   ├── test_risk_manager.py     # 38 tests
│   ├── test_strategy_engine.py  # 33 tests
│   └── ...
...
- `data/mes_4y.csv` — MES, 2021–2024 (~16 MB)
- `data/mnq_4y.csv` — MNQ, 2021–2024 (~17 MB)
...
- **Single strategy family.** Only EMA breakout strategies are profitable; momentum, mean reversion, and VWAP bounce are net-negative and not used in the primary configuration.
...
- **No intraday data feed.** Paper trading replays historical bars; it does not connect to a real-time feed.
...
- [ ] Complete 90-day paper validation with Telegram-alerted manual execution
