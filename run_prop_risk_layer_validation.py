...
from strategy.prop_risk_layer import PropRiskConfig, AccountMode
...
def run_dynamic_challenge(trades: list[Trade], label: str) -> MonteCarloResult:
    """Dynamic sizing with PropRiskLayer — symbol-aware config."""
    if label == "MES":
...
def run_funded_simulation(trades: list[Trade], label: str) -> MonteCarloResult:
    """Funded mode simulation — conservative preservation."""
...
def print_interpretation(rows: list[dict], funded_result: Optional[MonteCarloResult]):
    ...
    print(f"     - Combined/MNQ optimal: 0.35x flat → 63-68% pass rate")
    print(f"     - MES optimal: 0.50x + DD proximity → 68% @ 500 trades, 88% @ 1500")
    print(f"     - DD proximity HURTS high-variance instruments (MNQ) — creates recovery trap")
    ...
    if delta > 0.02:
        print(f"     YES — improvement of {delta:+.1%}")
    elif delta > -0.02:
        print(f"     COMPARABLE — within noise ({delta:+.1%})")
    else:
        print(f"     NO — pass rate decreased by {abs(delta):.1%}")

    print(f"\n  4. FAILURE ANALYSIS")
    bg = baseline["giveback_rate"]
    dg = dynamic["giveback_rate"]
    print(f"     Giveback rate: {bg:.1%} → {dg:.1%} ({dg - bg:+.1%})")
    bf20 = baseline["fail_within_20"]
    df20 = dynamic["fail_within_20"]
    print(f"     Early fail (<20 trades): {bf20:.1%} → {df20:.1%}")
    bf = baseline["fail_rate"]
    df = dynamic["fail_rate"]
    print(f"     Total fail rate: {bf:.1%} → {df:.1%} ({df - bf:+.1%})")
    bdd = baseline["mean_max_dd"]
    ddd = dynamic["mean_max_dd"]
    print(f"     Mean max DD: ${bdd:,.0f} → ${ddd:,.0f}")
    bp95 = baseline["p95_max_dd"]
    dp95 = dynamic["p95_max_dd"]
    print(f"     P95 DD: ${bp95:,.0f} → ${dp95:,.0f}")

    print(f"\n  5. TIMING")
    bat = baseline["avg_trades_to_pass"]
    dat = dynamic["avg_trades_to_pass"]
    print(f"     Avg trades to pass: {bat:.0f} → {dat:.0f}")
    bmt = baseline["median_trades_to_pass"]
    dmt = dynamic["median_trades_to_pass"]
    print(f"     Median trades to pass: {bmt:.0f} → {dmt:.0f}")
    ...
    print(f"\n  7. RECOMMENDATIONS")
    if dp > bp * 1.05:
        print(f"     USE the dynamic config — clear improvement")
    elif dp > bp * 0.95:
        print(f"     CONSIDER the dynamic config — similar pass rate with lower DD risk")
    else:
        print(f"     INVESTIGATE further — pass rate regressed, tune parameters")
    if dp > 0:
        attempts = 1.0 / dp
    ...
    try:
        ...
        for label in ["Combined", "MES", "MNQ"]:
            ...
            if not trades or len(trades) < 20:
                logger.warning("Skipping %s — too few trades (%d)", label, len(trades))
                continue
            ...
            # 4. Dynamic challenge — extended horizon (1500 trades)
...
def _save_comparison_equity_plot(
    ...
    plt.suptitle("Baseline vs Dynamic Risk Layer — Sample Equity Paths", fontsize=14)
    plt.tight_layout()
